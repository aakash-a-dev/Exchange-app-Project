...
export function Depth({ market }: {market: string}) {
    ...
    useEffect(() => {
        ...
        getDepth(market).then((d: { bids: any[]; asks: SetStateAction<[string, string][] | undefined>; }) => {    
            setBids(d.bids.reverse());
            setAsks(d.asks);
        });

        getTicker(market).then((t: { lastPrice: SetStateAction<string | undefined>; }) => setPrice(t.lastPrice));
        getTrades(market).then((t: { price: SetStateAction<string | undefined>; }[]) => setPrice(t[0].price));
